...
; Module Name:
;
;   TestStructuredExceptionHandling2.asm.
;
; Abstract:
;
;   The purpose of this module is to ensure our understanding of how structured
;   exception handling with regards to exceptions being raised in exception
;   handlers.
;
; Compilation:
;
...
;
;       ml64 /Fe"TestStructuredExceptionHandling2.exe" Zi /Zd /nologo /W3 TestStructuredExceptionHandling2.asm /link /Zi /entry:Main /subsystem:console
;
;--
...
altentry ExceptionHandlerAccessViolationFault
altentry ExceptionHandlerAccessViolationResume
...
; Routine Description:
;
;   This is the exception handler for ExceptionHandler.  It verifies that
;   the exception occurred at the RIP we expected, then updates the rip to
;   the relevant resume address, then returns ExceptionContinueSearch.
;
; Arguments:
;
...
;--
        ...
        LEAF_ENTRY ExceptionExceptionHandler, _TEXT$00
...
;
        ...
        mov     rax, offset ExceptionHandlerAccessViolationFault ; Load fault IP
...
;
        ...
        mov     r11, offset ExceptionHandlerAccessViolationResume
        mov     CxRip[r8], r11                  ; Save addr to rip offset.
        mov     eax, ExceptionContinueExecution ; Load return code.

Eh90:   ret

        LEAF_END ExceptionExceptionHandler, _TEXT$00
...
;   resumes.
;
;   It then generates its own access violation fault.
;
; Arguments:
;
;   ExceptionRecord (rcx) - Supplies a pointer to an exception record.
...
;--
        ...
        NESTED_ENTRY ExceptionHandler, _TEXT$00, ExceptionExceptionHandler
...
;
; Now, generate another fault.
;

        xor     r10, r10                        ; Clear r10.
        ALTERNATE_ENTRY ExceptionHandlerAccessViolationFault
        mov     r10, [r10]                      ; Dereference NULL pointer.
        ALTERNATE_ENTRY ExceptionHandlerAccessViolationResume

        mov     eax, ExceptionContinueExecution ; Load return code.

Eh90:   ret                                     ; Return.

        NESTED_END ExceptionHandler, _TEXT$00
