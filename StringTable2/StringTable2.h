...
typedef struct _Struct_size_bytes_(SizeOfStruct) _STRING_TABLE2 {
    ...
    // 256-bit, 32-byte struct.  This needs to be 32-byte aligned due to it
    // being backed by YMMWORD registers.
    //
    ...
    //
    // (320 bytes consumed.  Next alignment is 64 bytes.)
    //

    //
    // There are 192 bytes remaining (out of 512 bytes) in this structure.  The
    // last 16 bytes are reserved for an optional pointer to a function table,
    // and the string table destructor function pointer, which leaves us 176
    // bytes to play with.  How we use this depends on the various flags set for
    // the table (specified at creation time and captured in the Flags field).
    // The following union captures the various permutations for this remaining
    // space.
    //
    ...
    union {
        ...
        //
        // The CopyStringArray() routine is special-cased to allocate the
        // backing STRING_ARRAY structure plus the accommodating buffers in this
        // space if it can fit.
        //
        ...
        struct {
            ...
            //
            // Multiple tables can be linked together via this linked list
            // entry.
            //
        ...
        //
        // An optional array of opaque context pointers associated with each slot
        // entry stored in the table.
        //

        PVOID SlotContexts[16];

        //
        // (448 bytes consumed.  Next alignment is 64 bytes.)
        //
    ...
    //
    // Pointer to a structure of function pointers.  This is always the second
    // to last element in the struct.
    //
    ...
    // always placed at the end of the struct.
    //
