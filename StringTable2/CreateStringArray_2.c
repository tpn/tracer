...
--*/
{
    ...
    Start = NULL;
    PreviousDelimeter = NULL;
    Length.LongPart = 0;

    //
    // Enumerate over the string buffer, looking for delimeters, such that
    ...
    for (Index = 0; Index < String->Length; Index++) {
        ...
        //
        // If it's not a delimeter character, continue the search.
        //
        ...
        //
        // We've found a delimeter character, and need to determine if it's
        // depicting the valid end point of string, or if it's superfluous.
        // A superfluous delimeter is one that is at the very start of the
        // string, or one that immediately follows a previous delimeter (i.e.
        // "foo;;bar").
        //
        ...
        IsSuperfluous = (
            Char == Start || (
                PreviousDelimeter &&
                (Char - sizeof(*Char)) == PreviousDelimeter
            )
        );
        ...
        if (IsSuperfluous) {
            ...
            //
            // The delimeter is superfluous.  Mark it as our previous delimeter
            // and continue the loop.
            //

            PreviousDelimeter = Char;
            continue;
        }

        //
        // We've found a seemingly valid delimeter.  We need to determine where
        // the string started, which we can ascertain from either the start of
        // the string (e.g. String->Buffer) if the previous delimeter is null,
        // or previous delimeter + sizeof(*Char) if it is not null.
        //

        if (!PreviousDelimeter) {

            //
            // Use the start of the string buffer as the start for this string
            // element.
            //
            ...
            Start = PreviousDelimeter + sizeof(*Char);
        ...
        //
        // Update the previous delimeter to point at this delimeter.
        //
    ...
    // delimeted input string is not required to terminate the final string
    // element with a delimeter, thus, we need to check if there's a final
    // string here.
    //
    // There's a final string if any of the following conditions hold true:
    //
    //  - If previous delimeter is null (i.e. no delimeters seen).
    //  - Else, if previous delimeter was not final character of string.
    //
    ...
    if (!PreviousDelimeter) {
        ...
        //
        // No delimeter was seen, so the "delimeted string" passed to us on
        // input was simply a single string.  If this is the case, then there's
        // an invariant that count should be 0 at this point.  Assert that now.
        //

        ASSERT(Count == 0);

        //
        // Toggle the final flag to true and set the start pointer to the very
        // start of the input string buffer.
        //

        Final = TRUE;
        Start = String->Buffer;

    } else if (Char != PreviousDelimeter) {

        //
        // The string didn't end with a delimeter, which means a final string
    ...
    if (Final) {
        ...
        //
        // The end character will always be the final character referenced by
        // the string buffer at this point.
        //
        ...
        Length.LongPart = (ULONG)((ULONG_PTR)End - (ULONG_PTR)Start);
    ...
    // number of underlying elements and the required allocation sizes.  Check
    // the count now -- if it's 0, the user has failed to provide a usable input
    // string (e.g. ";;;;;;;;;").
    //
