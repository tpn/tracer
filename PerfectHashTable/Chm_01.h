...
//
// N.B. 1000 is a huge number; normally, solutions are found on average within
//      3 attempts, and there's a 99.9% chance a solution will be found by the
//      18th attempt.
//

#define GRAPH_SOLVING_ATTEMPTS_THRESHOLD 1000
    ...
    )
{
    ...
    switch (Graph->MaskFunctionId) {

        case PerfectHashTableModulusMaskFunctionId:

            MaskedEdge = Edge % NumberOfEdges;
            break;

        case PerfectHashTableAndMaskFunctionId:
        case PerfectHashTableXorAndMaskFunctionId:

            MaskedEdge = Edge & Graph->EdgeMask;
            break;

        case PerfectHashTableFoldOnceMaskFunctionId: {

            ULONG Mask;
            ULONG Extra;
            ULONG Shift;
            ULONG PopCount;
            ULONG InvertedEdgeMask;
            ULARGE_INTEGER Quad;
            ...
            InvertedEdgeMask = ~Graph->EdgeMask;

            Extra = Quad.LowPart;
            Extra &= InvertedEdgeMask;

            Shift = TrailingZeros(InvertedEdgeMask);
            Extra >>= Shift;
        ...
        case PerfectHashTableInvalidMaskFunctionId + 1:
    ...
    )
{
    ...
    //ASSERT(!FAILED(Table->Vtbl->Mask(Table, Edge, &MaskedEdge)));
