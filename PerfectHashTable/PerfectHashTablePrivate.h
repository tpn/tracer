...
typedef struct _Struct_size_bytes_(SizeOfStruct) _PERFECT_HASH_TABLE_CONTEXT {
    ...
    //
    // This counter is incremented every time the threshold for attempts at
    // solving a graph is exceeded and the solution attempt is reset with a
    // larger underlying table size.
    //

    ULONG NumberOfTableResizeEvents;

    //
    // This threshold is doubled every time a resize event occurs, such that
    // more attempts are made at solving tables with larger vertex sizes.
    //
...
PERFECT_HASH_TABLE_HASH PerfectHashTableHash03;
PERFECT_HASH_TABLE_HASH PerfectHashTableHash04;
